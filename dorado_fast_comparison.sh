...
BASE_OUTPUT_DIR="/DATA4/amishi/16Apr/dorado_fast_comparison"
...
for Q in "${Q_SCORES[@]}"; do
    ...
    for pod5 in "$POD5_DIR"/*.pod5; do
        ...
        UNALIGNED_BAM="$UNALIGNED_DIR/${name}_fast_basecall_q${Q}.unaligned.bam"
        ALIGNED_BAM="$ALIGNED_DIR/${name}_fast_basecall_q${Q}.aligned.sorted.bam"
...
for Q in "${Q_SCORES[@]}"; do
    ...
    for pod5 in "$POD5_DIR"/*.pod5; do
        ...
        UNALIGNED_BAM="$UNALIGNED_DIR/${name}_fast_methyl_q${Q}.unaligned.bam"
        ALIGNED_BAM="$ALIGNED_DIR/${name}_fast_methyl_q${Q}.aligned.sorted.bam"
...
for bam in "$BASE_OUTPUT_DIR"/dorado_fast_basecall_q*/unaligned/*.bam; do
    [ -f "$bam" ] || continue
...
for bam in "$BASE_OUTPUT_DIR"/dorado_fast_basecall_q*/aligned/*.bam; do
    [ -f "$bam" ] || continue
...
for bam in "$BASE_OUTPUT_DIR"/dorado_fast_methyl_q*/unaligned/*.bam; do
    [ -f "$bam" ] || continue
    count=$(samtools view -c "$bam")
    printf "  %-10s reads → %s\n" "$count" "$(basename $bam)" | tee -a "$LOG"
done
...
for bam in "$BASE_OUTPUT_DIR"/dorado_fast_methyl_q*/aligned/*.bam; do
    [ -f "$bam" ] || continue
    mapped=$(samtools view -c -F 4 "$bam")
    total=$(samtools view -c "$bam")
    printf "  %-10s mapped / %-10s total → %s\n" "$mapped" "$total" "$(basename $bam)" | tee -a "$LOG"
